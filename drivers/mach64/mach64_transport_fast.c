 ...
 *
 * MainDOB Mach64 driver — fast-transport C dispatcher (data plane).
 *
 * The kernel int 0x85 boomerang lands in mach64_fast_entry (asm shim in
 * mach64_fast_entry.asm), which marshals the 5 register args into a cdecl
...
/* Engine diagnostic probe (defined in main.c). */
...
int32_t mach64_fast_dispatch(uint32_t opcode,
                             ...
                             uint32_t a4, uint32_t caller_pid)
{
    ...
    mach64_current_caller_pid = (pid_t)caller_pid;
    ...
    /* Private diagnostic: hardware-fill probe + engine status readback. */
    case MACH64_DV_ENGINE_DIAG:
        if (!payload || payload_size < sizeof(mach64_engine_diag_t)) return DV_ERR_INVAL;
        mach64_engine_diag((mach64_engine_diag_t *)payload);
        return DV_OK;

 ...
 *
 * MainDOB Mach64 driver — IPC transport for the control plane.
 *
 ...
 * mach64_transport_fast.c + mach64_fast_entry.asm.
 *
...
static int dispatch_control(dob_msg_t *msg, dob_msg_t *reply)
{
    ...
    case DOBVC_OP_MODE_SET:
        ...
        rc = mach64_internal_mode_set((const dv_mode_t *)msg->payload);
    ...
    case DOBVC_OP_SUBSCRIBE:
        rc = mach64_subscribe(msg->arg0, msg->arg1);
        reply_set(reply, rc); return rc;
...
int mach64_transport_ipc_run(uint32_t port)
{
    for (;;)
    {
        dob_msg_t msg;
        ...
        {
            mach64_shutdown_for_detach();
            return 0;
        }

        mach64_current_caller_pid = msg.sender_pid;
